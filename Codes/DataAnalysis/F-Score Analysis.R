...
Company_F_Score_Fiscal_Year <- Company_F_Score_Fiscal_Year %>% mutate(
                                                                      Quarter=ifelse(Month<4,"Q3",
                                                                                     ifelse(Month<7,"Q4",
                                                                                            ifelse(Month<10,"Q1",
                                                                                                   ifelse(Month<13,"Q2")))))
...
Company_F_Score_Only <- left_join(Company_F_Score_Only,
                                  Company_F_Score_Fiscal_Year)
...
Company_F_Score_Returns <- left_join(Company_F_Score_Only,
                                     Company_Returns) 

Company_F_Score_Returns_Full <- left_join(Company_F_Score_Full,
                                          Company_Returns,by=c("Symbol","Year"))
...
#To group F-Score into high and low
Company_F_Score_Grouping <- Company_F_Score_Returns_Filtered %>% mutate(F.Score_Grouping = 
                                                                        ifelse(F.Score>6, "High_F.Score",
                                                                               ifelse(F.Score<4,"Low_F.Score","Neither")))
...
Company_F_Score_Grouping_Only_Volume_Filtered <- anti_join(Company_F_Score_Grouping_Only,
                                                           Company_Low_Trading_Volume)

Company_F_Score_Returns_Volume_Filtered <- anti_join(Company_F_Score_Returns_Filtered[,c(1,3,4,6)]
                                                     ,Company_Low_Trading_Volume,
                                                     by=c("Year","Symbol"))
...
Company_F_Score_Grouping_Only_Volume_Filtered_PE <- left_join(Company_F_Score_Grouping_Only_Volume_Filtered,
                                                              PE_Ratio_Clean
                                                              ,by=c("Year","Symbol"))

Company_F_Score_Returns_Volume_Filtered_PE <- left_join(Company_F_Score_Returns_Volume_Filtered,
                                                        PE_Ratio_Clean, 
                                                        by=c("Year","Symbol"))
...
Company_F_Score_Returns_Volume_Filtered_PE_Industry <- left_join(Company_F_Score_Returns_Volume_Filtered_PE,
                                                                 Company_Industry_Clean,
                                                                 by="Symbol")
...
Company_F_Score_Returns_Volume_Filtered_PE_Industry_Classes <- left_join(Company_F_Score_Returns_Volume_Filtered_PE_Industry,
                                                                         Company_F_Score_Classes)
...
ANOVA_Base_Group <- aov(Holding_Return ~ F.Score_Grouping,
                        data = Company_F_Score_Grouping_Only_Volume_Filtered)
...
#Hypothesis 4 Plots
Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry_Split1 <- subset(Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry, 
                                                                           Sector=="Basic Industries"|Sector=="Public Utilities"|Sector=="Basic Industries" |Sector=="Consumer Non-Durables"|Sector=="Energy")

Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry_Split2 <- subset(Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry, 
                                                                           Sector=="Health Care"|Sector=="Miscellaneous"|Sector=="Capital Goods"|Sector=="Technology"|Sector=="Transportation"|Sector=="Consumer Services"|Sector=="Consumer Durables")

ScatterPlot_Group_Sector_Part1 <-ggplot (Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry_Split1,aes(x=F.Score_Grouping,y=Holding_Return,color=Sector))+
                                          geom_point()+
                                          geom_jitter()+
                                          facet_grid(~Sector)+
                                          stat_summary(fun.y=mean,geom="line",lwd=2,aes(group=1))+
                                          scale_x_discrete(labels=c("High","Low"))+ 
                                          scale_color_manual(values=wes_palette(n=5, name="Rushmore"))

ScatterPlot_Group_Sector_Part2 <-ggplot (Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry_Split2,aes(x=F.Score_Grouping,y=Holding_Return,color=Sector))+
                                          geom_point()+
                                          geom_jitter()+
                                          facet_grid(~Sector)+
                                          stat_summary(fun.y=mean,geom="line",lwd=2,aes(group=1))+
...
#Hypothesis 5 Plots
ScatterPlot_Group_Year <-ggplot (Company_F_Score_Grouping_Only_Volume_Filtered_PE_Industry,aes(x=F.Score_Grouping,y=Holding_Return,color=Year))+
                                  geom_point()+
                                  facet_grid(~Year)+  
                                  geom_jitter()+
                                  stat_summary(fun.y=mean,geom="line",lwd=2,aes(group=1))+
